...
def test_resolves_from_env(monkeypatch):
    monkeypatch.setenv("DAGSTER__MYDB__PASSWORD", "secret123")
...
def test_case_insensitive_keys(monkeypatch):
    monkeypatch.setenv("DAGSTER__STRIPE__API_KEY", "sk_test_abc")
    resolver = DevEnvSecretResolver()
...
def test_raises_on_missing_env():
    ...
    with pytest.raises(RuntimeError, match="DAGSTER__NONEXISTENT__KEY"):
        resolver.resolve_as_str(ref)


def test_error_message_includes_export_hint():
    resolver = DevEnvSecretResolver()
    with pytest.raises(RuntimeError, match="export DAGSTER__"):
